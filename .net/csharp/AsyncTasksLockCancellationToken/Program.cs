...
public static class Program
{
    ...
    public async static Task Main(string[] args)
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            tasks.Add(Task.Run(() => IncrementCounter(i), cancellationToken)); // PASSANDO O CANCELLATION TOKEN
        }

        Thread.Sleep(10); // Aguarda um pouco para cancelar o token

        cancellationTokenSource.Cancel(); // Cancela o Token pelo Source, assim todos os tokens são cancelados
