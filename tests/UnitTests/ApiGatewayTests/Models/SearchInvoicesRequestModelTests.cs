...
namespace IDSIM.Services.UnitTest.ApiGatewayTests.Models
{
    ...
    public class SearchInvoicesRequestModelTests : IClassFixture<UnitApiTestsFixture>
    {
        private readonly UnitApiTestsFixture _unitApiTestsFixture;

        public SearchInvoicesRequestModelTests(UnitApiTestsFixture unitApiTestsFixture)
        {
        ...
        {
            // Arrange
            var validModel = _unitApiTestsFixture.GetValidSearchInvoicesRequestModel();
        ...
        {
            // Arrange
            var validModel = _unitApiTestsFixture.GetInvalidSearchInvoicesRequestModel();

            // Act
            var facadeResponse = validModel.Validate<It.IsAnyType>();

            // Assert
            Assert.Equal(6, facadeResponse.Errors?.Count);
        }
    }
}
